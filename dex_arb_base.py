...
async def strategy(subscriber: aioprocessing.AioQueue,
    ...
    async def _process_pending_order(pending: Pending):
        ...
        if min_amount_in_usdt <= max_bet_size:
            ...
            # added info 
            pending_info['simulated_profit_in_usdt'] = simulated_profit_in_usdt
            pending_info['final_profit'] = final_profit
            string_pending_info = {key: str(value) for key, value in pending_info.items()}
            await influxdb.send('PENDING_INFO', string_pending_info)
    ...
    while True:
        try:
            ...
            if data_type == 'setup':
                ...
                # print(len(spreads.keys()))
                # def reduce_dict_to_100_keys(input_dict):
                #     # Convert dictionary to a list of (key, value) pairs
                #     items = list(input_dict.items())
                    
                #     # Keep only the first 100 items
                #     reduced_items = items[:100]
                    
                #     # Convert back to dictionary
                #     reduced_dict = dict(reduced_items)
                    
                #     return reduced_dict

                # reduced_spreads = reduce_dict_to_100_keys(spreads)
